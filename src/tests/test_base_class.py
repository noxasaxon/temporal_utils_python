...
from temporal_utils.base_class import BaseTemporalActivity, TemporalActivityValidators
...
def test_activity_has_its_own_call_options():
    class ActivityWithCallOptions(BaseTemporalActivity):
        @activity.defn
...
def test_activity_doesnt_have_its_own_call_options():
    with pytest.raises(
        ...
        class ActivityWithoutOptions(BaseTemporalActivity):
            @activity.defn
...
def test_activity_succeeds_with_exactly_one_input_arg():
    class ActivityWithOneArg(BaseTemporalActivity):
        @activity.defn
...
def test_activity_fails_with_more_than_one_input_arg():
    with pytest.raises(
        ...
        class ActivityWithTooManyInputs(BaseTemporalActivity):
            @activity.defn
...
def test_activity_fails_with_no_input_arg():
    with pytest.raises(
        ...
        class ActivityWithNoInputArgs(BaseTemporalActivity):
            @activity.defn
...
def test_activity_fails_with_when_arg_isnt_pydantic():
    with pytest.raises(
        ...
        class ActivityWithArgDoesntJSONSerializeToDict(BaseTemporalActivity):
            @activity.defn
